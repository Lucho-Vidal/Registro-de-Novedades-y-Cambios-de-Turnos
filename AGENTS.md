...
- Password recovery: login has "¿Olvidó su contraseña?" → `AuthService.resetear_password(username)` derives the user's email as `username + correo_dominio` (config `correo_dominio`, default `@trenesargentinos.gob.ar`), sets a temp password via `secrets.token_urlsafe(9)`, marks `debe_cambiar_clave=1`, audits `resetear_clave`/`usuarios`, and returns `(email, temporal)`; `login_view` sends it via `enviar_informe_outlook`. Password expiry: `usuarios.password_cambiado_en` + `debe_cambiar_clave` (migrated via PRAGMA/ALTER in `initialize()`); `AuthService.requiere_cambio_clave(id)` is True when the flag is set or when `clave_expiracion_dias` (config, default 90; 0 = off) has elapsed since `password_cambiado_en` (NULL counts as expired). On login, `LoginView.login` shows a mandatory change dialog (new + confirm, via `cambiar_password`) and won't enter the app until changed. `cambiar_password` sets `password_cambiado_en=now` and clears the flag; `crear_usuario`/`crear_administrador_inicial` set `password_cambiado_en=now`. Period + domain are edited in `Administración > Contraseñas y recuperación` (`admin_views.mostrar_configuracion_clave`, permiso `sesion.configurar`).
